...
function twentyseventeen_scripts() {
    ...
    wp_enqueue_script('jquery-mobile-js', 'https://code.jquery.com/mobile/1.4.5/jquery.mobile-1.4.5.min.js', array());
...
if (class_exists('WP_Customize_Control')) {
    ...
    class WP_Customize_Range extends WP_Customize_Control {
        ...
        public function render_content() {
            ?>
            <label>
                <span for="price-min"><?php echo esc_html($this->label); ?>:</span>
...
//        'taxonomies' => array('category', 'post_tag'),
    ...
    register_post_type('gallery-port-type', $args);
}

add_action('init', 'gallery_custom_post_type');

// Add the Meta Box
function gallery_custom_meta_box() {
    add_meta_box(
            'custom_meta_box', 'Gallery Fields', 'gallery_meta_callback', 'gallery-port-type', 'normal', 'high'
    );
}

add_action('add_meta_boxes', 'gallery_custom_meta_box');

function gallery_metabox_enqueue($hook) {
    if ('post.php' == $hook || 'post-new.php' == $hook) {
        wp_enqueue_script('gallery-metabox', get_template_directory_uri() . '/inc/gallery-metabox/js/gallery-metabox.js', array('jquery', 'jquery-ui-sortable'));
        wp_enqueue_style('gallery-metabox', get_template_directory_uri() . '/inc/gallery-metabox/css/gallery-metabox.css');
    }
}
...
function create_shortcode($args) {
    ...
    $img = wp_get_attachment_image($args['image'], ' medium');
    $content = '<p>' . $args['content_port'] . '</p>';
...
function your_name_integrateWithVC() {
   vc_map( array(
      ...
      "base" => "bartag",
      "class" => "",
      "category" => __( "My Shortcode", "my-text-domain"),
