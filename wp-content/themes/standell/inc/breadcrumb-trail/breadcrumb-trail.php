...
class Breadcrumb_Trail {
    ...
    public function default_labels() {
        ...
        $labels = array(
            'browse' => esc_html__('Browse:', 'standell'),
            'home' => esc_html__('Home', 'standell'),
            'error_404' => esc_html__('404 Not Found', 'standell'),
            'archives' => esc_html__('Archives', 'standell'),
            /* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
            'search' => esc_html__('Search results for &#8220;%s&#8221;', 'standell'),
            /* Translators: %s is the page number. */
            'paged' => esc_html__('Page %s', 'standell'),
            /* Translators: Minute archive title. %s is the minute time format. */
            'archive_minute' => esc_html__('Minute %s', 'standell'),
            /* Translators: Weekly archive title. %s is the week date format. */
            'archive_week' => esc_html__('Week %s', 'standell'),
    ...
    public function themesflat_do_trail_items() {
        ...
        /* If not viewing the front page. */ else {
            ...
            /* If viewing a single post. */ elseif (is_singular()) {
                $this->themesflat_do_singular_items();
            }

            /* If viewing an archive page. */ elseif (is_archive()) {
            ...
            /* If viewing the 404 page. */ elseif (is_404()) {
    ...
    public function themesflat_do_front_page_items() {
        ...
        if (true === $this->args['show_on_front'] || is_paged() || ( is_singular() && 1 < get_query_var('page') )) {
            ...
            /* If on the main front page, add the network home link item and the home item. */ else {
                $this->themesflat_do_network_home_link();
    ...
    public function themesflat_do_post_hierarchy($post_id) {
        ...
        /* If the post type has rewrite rules. */ elseif (false !== $post_type_object->rewrite) {
    ...
    public function themesflat_do_minute_hour_archive_items() {
        ...
        if (true === $this->args['show_title'])
            $this->items[] = sprintf($this->args['labels']['archive_minute_hour'], get_the_time(_x('g:i a', 'minute and hour archives time format', 'standell')));
    }
    ...
    public function themesflat_do_minute_archive_items() {
        ...
        /* Add the minute item. */
        if (true === $this->args['show_title'])
            $this->items[] = sprintf($this->args['labels']['archive_minute'], get_the_time(_x('i', 'minute archives time format', 'standell')));
    }
    ...
    public function themesflat_do_hour_archive_items() {
        ...
        /* Add the hour item. */
        if (true === $this->args['show_title'])
            $this->items[] = sprintf($this->args['labels']['archive_hour'], get_the_time(_x('g a', 'hour archives time format', 'standell')));
    }
    ...
    public function themesflat_do_day_archive_items() {
        ...
        $year = sprintf($this->args['labels']['archive_year'], get_the_time(_x('Y', 'yearly archives date format', 'standell')));
        $month = sprintf($this->args['labels']['archive_month'], get_the_time(_x('F', 'monthly archives date format', 'standell')));
        $day = sprintf($this->args['labels']['archive_day'], get_the_time(_x('j', 'daily archives date format', 'standell')));
    ...
    public function themesflat_do_week_archive_items() {
        ...
        $year = sprintf($this->args['labels']['archive_year'], get_the_time(_x('Y', 'yearly archives date format', 'standell')));
        $week = sprintf($this->args['labels']['archive_week'], get_the_time(_x('W', 'weekly archives date format', 'standell')));
    ...
    public function themesflat_map_rewrite_tags($post_id, $path) {
        ...
        if (is_array($matches)) {
            ...
            foreach ($matches as $match) {
                ...
                if ('%year%' == $tag)
                    $this->items[] = '<a href="' . get_year_link(get_the_time('Y', $post_id)) . '">' . sprintf($this->args['labels']['archive_year'], get_the_time(_x('Y', 'yearly archives date format', 'standell'))) . '</a>';

                /* If using the %monthnum% tag, add a link to the monthly archive. */
                elseif ('%monthnum%' == $tag)
                    $this->items[] = '<a href="' . get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id)) . '">' . sprintf($this->args['labels']['archive_month'], get_the_time(_x('F', 'monthly archives date format', 'standell'))) . '</a>';

                /* If using the %day% tag, add a link to the daily archive. */
                elseif ('%day%' == $tag)
                    $this->items[] = '<a href="' . get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id)) . '">' . sprintf($this->args['labels']['archive_day'], get_the_time(_x('j', 'daily archives date format', 'standell'))) . '</a>';
...
class bbPress_Breadcrumb_Trail extends Breadcrumb_Trail {
    ...
    public function themesflat_do_trail_items() {
        ...
        elseif (bbp_is_topic_tag_edit()) {
            ...
            if (true === $this->args['show_title'])
                $this->items[] = esc_html__('Edit', 'standell');
        }
        ...
        /* If viewing a single forum. */ elseif (bbp_is_single_forum()) {
