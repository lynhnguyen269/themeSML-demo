...
class Themesflat_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct(
                'widget_recent_post', esc_html__('themesflat - Recent Post', 'standell'), array(
            'classname' => 'widget-recent-news',
            'description' => esc_html__('Recent Post.', 'standell')
                )
        );
    }
    ...
    function widget($args, $instance) {
        ...
        <ul class="<?php echo esc_attr(implode(' ', $classes)); ?> clearfix">  
            ...
            <?php else : ?>  
                <?php printf('<li>%s</li>', esc_html__('Oops, category not found.', 'standell')); ?>
            <?php endif; ?>        
        </ul>		
        <?php
    ...
    function form($instance) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('category')); ?>"><?php esc_html_e('Select Category:', 'standell'); ?></label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('category')); ?>" name="<?php echo esc_attr($this->get_field_name('category')); ?>[]">
                <option value=""<?php selected(empty($instance['category'])); ?>><?php esc_html_e('All', 'standell'); ?></option>
                <?php
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('ids')); ?>"><?php esc_html_e('Get Post by IDS EX:1,2,3', 'standell'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Count:', 'standell'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_thumbnail')); ?>"><?php esc_html_e('Show Thumbnail ?', 'standell') ?></label>
        </p>  
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_content')); ?>"><?php esc_html_e('Show Content ?', 'standell') ?></label>
        </p>       
        <p>
            <input class="checkbox" type="checkbox" <?php checked($show_date); ?> id="<?php echo esc_attr($this->get_field_id('show_date')); ?>" name="<?php echo esc_attr($this->get_field_name('show_date')); ?>" />
            <label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>"><?php esc_html_e('Show Date ?', 'standell'); ?></label>
        </p>
        <?php
    }
